#!/usr/bin/env bash
# 放到 scripts/publish-cnb.sh。把 REPO 改成你的 <CNB_SLUG>（如 org/app）。
...
if git -C "$tmp/repo" diff --cached --quiet; then echo "manifest 无变化"; else
