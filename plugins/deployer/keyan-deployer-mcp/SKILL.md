...
安装 `deployer@dacheng` 插件后，Claude Code 会自动注册 `keyan-deployer-mcp` MCP server（通过 `npx -y @dachengzhihui/keyan-deployer-mcp`）。你只需提供环境变量即可。
...
1. **Shell 环境继承（推荐）**：在启动 Claude Code 的终端里 export：
   ```bash
...
2. **手动覆盖注册**：如果插件自动注册的 server 缺少 env，可以重新添加同名 server：
   ```bash
...
3. **编辑 Claude Code MCP 配置**：在 `~/.claude/settings.json` 的 `mcpServers` 中为 `keyan-deployer-mcp` 补 `env`。

> 若环境变量未设置，MCP server 启动会打印 `Missing required env var: KEYAN_API_URL` / `DEPLOY_PASSWORD`，属正常提示。
